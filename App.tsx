...
const App: React.FC = () => {
  ...
  const handleCreateTask = async (formData: any) => {
    ...
    try {
      const response = await fetch(APP_CONFIG.SCRIPT_URL, {
        method: 'POST',
        body: JSON.stringify({ action: 'createNewTask', ...formData })
      });
      const res = await response.json();
      if (res.success) {
  ...
  const handleUpdateUser = async (email: string, updates: Partial<User>) => {
    try {
      ...
      writeLog(LogAction.UPDATE_STATUS, `更新使用者: ${email}`);
  ...
  const handleViewHistory = (taskUid: string) => {
